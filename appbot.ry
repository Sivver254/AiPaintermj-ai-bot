...
async def start(m: Message):
    await m.answer(
        f"Привет, {hbold(m.from_user.full_name)}!\n{HELP}",
...
@router.message(F.text == "📷 Реализм")
async def style_realism_hint(m: Message):
    await m.answer("Напиши промпт для стиля «Реализм». Пример: «закат над океаном»", reply_markup=main_kb())

@router.message(F.text == "🌸 Аниме")
async def style_anime_hint(m: Message):
    await m.answer("Напиши промпт для стиля «Аниме». Пример: «герой с синей аурой»", reply_markup=main_kb())

@router.message(F.text == "🧩 Пиксель-арт")
async def style_pixel_hint(m: Message):
    await m.answer("Напиши промпт для стиля «Пиксель-арт». Пример: «рыцарь в подземелье»", reply_markup=main_kb())
...
async def imagine(m: Message):
    ...
    # Сейчас w/h/steps/cfg не пробрасываем в Horde.generate для простоты MVP,
    # позже добавим; пока используем дефолты.
...
# Просто текст без команд — генерим по умолчанию в стиле "реализм"
